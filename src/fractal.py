...
#                print("sending normal !", normal.shape, type(normal), normal.dtype)
                ...
                if potential_dic["kind"] == "infinity":
                    ...
                    #damping = 0.75
                    n_iter_fl = post_dic.get("n_iter", 10)
                    swirl_fl = post_dic.get("swirl", 1.)
                    t = [np.angle(z_norm)]# * d ** skip]
                    val = np.zeros_like(t)
#                    valk = 0.
                    di = 1.
                    ...
                    # Convergence of a geometric serie
                    # n_cv_001 = int(np.floor(np.log(0.01) / np.log(damping)))
...
#                        val2 += di * (np.cos(t[i-1] + dphi) + (
#                                  k_alpha + k_beta * d**nu_frac) * (
#                                  np.cos(t[i] + dphi) - np.cos(t[i-1] + dphi)))
                    #val = val + val2
#                    val = np.arcsin(val / valk)
#                    val = np.arccos(val / valk) #+ 0.5 * np.arcsin(val / valk)
                    # val = np.sin(val / valk)
                    del t
