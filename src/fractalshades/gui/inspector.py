# -*- coding: utf-8 -*-
    ...
    class Method_inspector:
        ...
        def build_dict(self):
            ...
            """
            md = self.method_dict = dict()
            sign = inspect.signature(getattr(self._class, self._method))
            md["n_params"] = len(sign.parameters.items())
            i_param = 0
        ...
        def insert_param(self, i_param, name, param):
            ...
            md[(i_param, "has_def")] = has_def
            if has_def:
                md[(i_param, "val_def")] = param.default
            md[(i_param, "name")] = name

            # inspect the datatype - is it a union
            ptype = param.annotation
            uargs = typing.get_args(ptype)
            md[(i_param, "n_union")] = len(uargs)
            if len(uargs) != 0:
        ...
        def insert_uarg(self, i_param, i_union, uarg):
            ...
            if dataclasses.is_dataclass(utype):
                ...
                md[(i_param, i_union, "has_fields")] = True
                md[(i_param, i_union, "type")] = utype.__name__
                for ifield, field in enumerate(dataclasses.fields(utype)):
                    self.insert_field(i_param, i_union, ifield, field)
            else:
                md[(i_param, i_union, "has_fields")] = False
                md[(i_param, i_union, "type")] = utype.__name__
                md[(i_param, i_union, "val")] = ""
...
#            md[(i_param, i_union, ifield, "type")] = field.__name__
            ...
            # fill in md dict
            md[(i_param, i_union, ifield, "has_default")
                ] = has_field_default
            md[(i_param, i_union, ifield, "name")] = fname # placeholder
            md[(i_param, i_union, ifield, "ftype")] = ftype # placeholder
            if has_field_default:
                md[(i_param, i_union, ifield, "default")] = default
            md[(i_param, i_union, ifield, "val")] = "" # placeholder
        ...
        def flatten(self):
            ...
            QAbstractItemModel
            """
            self.arr = tuple(self.method_dict.keys())
            self.mapping = dict(zip(range(len(self.method_dict)),
                                    tuple(self.method_dict.keys())))
