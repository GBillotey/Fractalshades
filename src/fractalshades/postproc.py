...
#    @property
#    def zn_iterate(self):
#        """ Used for orbit-based postprocessing """
#        return self.batch.fractal.zn_iterate
#
#    @property
#    def calc_name(self):
#        return self.batch.calc_name
...
#             ) = Continuous_iter_pp_infinity(c_pt, zn, d, a_d, M, zn_iterate)
#            n += delta_n + back_iter
#
...
#        """ Enable standard Python serialization mechanism
#        """
#        args = (self.fractal, self.calc_name, self.key, self._func, self.inv) # self.fractal
#        return (self.__class__, args, None, None, None, None)
#
#    def __getitem__(self, chunk_slice):
#        """ Returns the raw array, with self.func applied if not None """
...
#        
#@numba.njit(nogil=True, fastmath=True)
#def Continuous_iter_pp_infinity(c, zn, d, a_d, M, zn_iterate):
...
#            zn2[i] = zn_iterate(zn2[i], c[i])
#            zn_orbit[i] = zn_iterate(zn_orbit[i], c[i])
#            delta_n[i] += 1
#            count += 1
#
#    k = np.abs(a_d) ** (1. / (d - 1.))
...
#    c_pt, zn, nu_frac, n_iter_fl, k_fl, phi_fl, zn_iterate
#):
#    # if potential_dic["kind"] == "infinity":
#    # C1-smooth phase angle for z -> a_d * z**d
#    # G. Billotey
...
#        for i in range(n_iter_fl):
#            val_loc = (
#                a0 * np.sin(orbit_angle[j, i] + phi_fl[i])
#                + a1 * np.sin(orbit_angle[j, i + 1] + phi_fl[i])
#                + a2 * np.sin(orbit_angle[j, i + 2] + phi_fl[i])
#                + a3 * np.sin(orbit_angle[j, i + 3] + phi_fl[i])
#            ) * k_fl[i]
#            val[j] += val_loc
#
#    return val
...
#
#@numba.njit(nogil=True, fastmath=True)
#def Fieldlines_pp_infinity_legacy(
